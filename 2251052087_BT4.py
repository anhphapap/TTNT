...
print("Hiển thị 10 chữ số đầu tiên")
plot_digits(X,y)
...
print ("\nHiền thị một số chữ số huấn luyện với nhãn cụm dự đoán:")
...
# Kết quả sẽ giống với train_cluster_labels vì dữ liệu đã được huấn luyện
...
print(f"Chi sõ Adjusted Rand Index cho tập huãn luyện: {train_ari}")
...
plot_kmeans_decision_boundary(X_train, train_cluster_labels, kmeans, title="Vùng phần cụm k-means trên dữ liệu huấn luyện")